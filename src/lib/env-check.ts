export function checkEnvVariables() {
  ...
  if (process.env.NODE_ENV === 'production') {
    ...
    const directUrl = process.env.MYSQL_DIRECT_URL || '';

    if (!databaseUrl.startsWith('mysql://')) {
      throw new Error('MYSQL_DATABASE_URL must use a MySQL connection string in production.');
    }

    if (!directUrl.startsWith('mysql://')) {
      throw new Error('MYSQL_DIRECT_URL must use a MySQL connection string in production.');
    }
  }
}
