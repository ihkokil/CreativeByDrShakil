...
export default function CourseDetailClient({ initialCourse, initialTeachers, initialCurriculum, slug }: Props) {
    const router = useRouter();
    const [course, setCourse] = useState<Course | null>(initialCourse);
    const [teachers, setTeachers] = useState<PublicTeacher[]>(initialTeachers);
    const [dynamicCurriculum, setDynamicCurriculum] = useState<CurriculumNode[]>(initialCurriculum);
    ...
    const onEnterCourse = async () => {
        ...
        try {
            const res = await fetch(`/api/study/courses/${slug}/progress`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ lessonNodeId: firstLesson.id }),
            });
            if (res.ok) router.push(`/study/${slug}`);
                ...
                <div className={styles.leftContent}>
                    ...
                    {dynamicCurriculum.length > 0 && (
                        <section className={styles.section}>
                            ...
                            <CourseLessonList curriculum={dynamicCurriculum} hasAccess={userEnrolled} courseSlug={slug} />
                        </section>
                    )}
